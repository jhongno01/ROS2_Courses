...
private:
    ...
    DriveOutput & output) const
  {
    ...
    const double front_ratio = clamp_value(
      ...
      0.0,
      1.0);
    const double speed_cap = output.costmap.narrow_corridor ?
      std::max(min_linear_speed_, narrow_corridor_speed_m_s_) : max_linear_speed_;
    ...
    for (int i = 0; i < sample_count; ++i) {
      const double ratio = sample_count == 1 ?
        0.0 : static_cast<double>(i) / static_cast<double>(sample_count - 1);
    ...
    std::vector<Pose2D> & path) const
  {
    ...
    const double heading_error = safety.heading_reference_valid ?
      std::fabs(normalize_angle_rad(safety.current_heading_rad + pose.yaw - safety.heading_reference_rad)) : 0.0;
